...
3. *What is the distribution of the maximum and minimum of the flow?*
...
Note that the original question is How did the range of the flow change in a year? I revised it a little bit to make it more suitable and clear for the following analysis. 

Q2.1 *Summarizing 3*: Here I create a categorical variable with 3 or more groups from an existing numerical variable. 
Generally speaking, this does not help for the question without any further theoretical backgrounds. Although 3 groups are created, the cutting points are arbitrary.
...
**Q3 What is the distribution of the maximum and minimum of the flow?**
...
Q3.2 *Graphing 5*
```{r}
# #not reasonable, just for demonstrating that task 5 and 7 could be done in some way.
# flow_sample %>%
#   filter(extreme_type=="maximum")%>%
#     ggplot(aes(flow))+
#     geom_density()+
#     geom_blank(alpha=0.8)
...
flow_sample %>%
  filter(extreme_type=="maximum")%>%
  group_by(month)%>%
  summarise(maximum_mean = mean(flow,na.rm=T),maximum_median=median(flow,na.rm = T),maximum_sd=sd(flow,na.rm = T), maximum_n = length(flow))


```
Q4.2 *Graphing 5*
```{r}
# ggplot(flow_sample,aes(month))+
#   geom_bar(aes(fill=extreme_type),alpha=0.3)

maximum_mean<- as.numeric(flow_sample %>%
  filter(extreme_type =="maximum")%>%
  summarise(mean(flow)))
...
flow_sample %>%
  ...
  geom_boxplot()+
  geom_hline(yintercept = maximum_mean)
...
Considering the definition of tidy data, my data are tidy. As the `glimpse()`and `head`shows, Each row is an **observation** for this station at the specific time. and Each column is a **variable**: Station_id is a categorical variable to recognize the station; Year, month, and day are the variables for the date; extreme_type is a categorical variable to distinguish the type of flow; sym is a a categorical variable, but there isn't any further description. As a result, Each cell is a **value** for a specific variable.
As a result, our flow_sample is untidy. 
